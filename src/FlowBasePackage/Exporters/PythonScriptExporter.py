...
def nodeToScript(node, supportedDataTypes, supportedStructures):
    ...
    script += (
        "{0} = getRawNodeInstance('{1}', packageName='{2}', libName={3})\n".format(
...
class PythonScriptExporter(IDataExporter):
    ...
    name_filter = "uflow program scripts (*.py)"
        ...
        if openFilename != "":
            with open(openFilename, "r") as f:
                ...
                if (
                    ...
                    uflowInstance.newFile()
                    ROOT_GRAPH = uflowInstance.graphManager.get().findRootGraph()
                    mem["createScene"](ROOT_GRAPH)
                    uflowInstance.fileBeenLoaded.emit()
    ...
    def doExport(uflowInstance):
        ...
        script = "# -*- coding: utf-8 -*-\n\n"
        script += (
            "# This file was auto-generated by uflow exporter '{0} v{1}'\n".format(
        ...
        script += "from uflow import getRawNodeInstance\n\n"
        ...
        rootGraph = uflowInstance.graphManager.get().findRootGraph()

        if len(rootGraph.getNodesList()) == 0:
            QMessageBox.warning(uflowInstance, "Warning", "Nothing to export!")
            return
        ...
        try:
                ...
                for outPin in node.outputs.values():
                    for inPin in getConnectedPins(outPin):
                        graphScript += "{0} = ROOT_GRAPH.graphManager.findPinByName('{1}')\n".format(
                            outPin.getFullName(), outPin.getFullName()
                        )
                        graphScript += "{0} = ROOT_GRAPH.graphManager.findPinByName('{1}')\n".format(
                            inPin.getFullName(), inPin.getFullName()
                        )
                        graphScript += "connectPins({0}, {1})\n".format(
                            outPin.getFullName(), inPin.getFullName()
                        )
            ...
            QMessageBox.warning(uflowInstance, "Warning", str(e))
