...
from uflow.UI.Widgets.PropertiesFramework import (
    CollapsibleFormWidget,
    PropertiesWidget,
)
from uflow.UI.Canvas.UICommon import clearLayout
from uflow.UI.Widgets.QtSliders import pyf_ColorSlider, pyf_Slider
from uflow.UI.Utils.stylesheet import editableStyleSheet, ConnectionTypes
from uflow.UI.Widgets.PreferencesWindow import *
import uflow.UI as UIModule
            ...
            if isinstance(obj, QtGui.QColor):
                ...
                if name in [
                    ...
                    inp.valueChanged.connect(
                        lambda color,
                        name=name,
                        update=False: editableStyleSheet().setColor(name, color, update)
                    )
                    ...
                    inp.stateChanged.connect(
                        lambda color,
                        name=name,
                        update=True: editableStyleSheet().setColor(name, color, update)
                    )
                    ...
                    inp.currentIndexChanged.connect(
                        lambda value,
                        name=name,
                        update=False: editableStyleSheet().setColor(name, value, update)
                    )
                elif name in [
                    "LOD_Number",
                    "NodeSwitch",
