...
from uflow.Core.Common import SingletonDecorator
...
class LoggerTool(DockTool):
    ...
    def flushPython(self):
        self.logView.moveCursor(QtGui.QTextCursor.End, QtGui.QTextCursor.MoveAnchor)
        self.logView.moveCursor(QtGui.QTextCursor.Up, QtGui.QTextCursor.MoveAnchor)
        self.logView.moveCursor(
            QtGui.QTextCursor.StartOfLine, QtGui.QTextCursor.MoveAnchor
        )
        self.logView.moveCursor(QtGui.QTextCursor.End, QtGui.QTextCursor.KeepAnchor)
    ...
    def anchorClickedMethod(self, url):
        if os.path.exists(url.url().split("::")[0]):
            editCmd = ConfigManager().getPrefsValue(
                "PREFS", "GeneralPreferences/EditorCmd"
            )
            editCmd = editCmd.replace("@FILE", url.url().replace("::", ":"))
            subprocess.Popen(editCmd)
        else:
            man = self.uflowInstance.graphManager
            node = man.get().findNode(url.url())
            if node:
                self.uflowInstance.getCanvas().clearSelection()
                node.getWrapper().setSelected(True)
                self.uflowInstance.getCanvas().frameSelectedNodes()
